...
description: Triage and fix Bugsee crashes, errors, and bug reports using the Bugsee MCP server. Use when asked to fix a Bugsee issue, debug a crash or exception, investigate a bug report, root-cause a production issue, or when given a Bugsee issue key like MYAPP-123.
license: MIT
category: workflow
...
This skill needs the Bugsee MCP server connected. It is read-only (scope `mcp:read`) and exposes three tools: `list_applications`, `list_issues`, and `get_issue`.
...
Read the timing, environment, exception details, stack frames, breadcrumbs, network events, and logs.
...
- The Bugsee MCP is read-only — it cannot close the issue. Tell the user to verify the fix in a new build and resolve the issue from the dashboard.
